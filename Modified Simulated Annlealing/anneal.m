function [minimum, fval, rejs, xTracker, fvalTracker, All_energies, acc_energies, acc_it , last_BAccR, sR, sBlk,sBlue, Max_S, Min_S, rf_accepted, back_acc_count] = anneal(loss, parent, options, d, bias ) 
...
InitS=4;  % Initial Step Size.  Step ranges from -1 to +1  or +-5
r =[0:0.00000001:0.1, 0.9:0.00000001:1];  % Hollow Step Size Range  0.00000001

StepDecayFactor= 0.98; %0.97 %0.89
TempDecayFactor= 0.98; %0.89
...
def = struct(...
        ...
        'Generator',@(p,q) (p + q), ...
        'StepSize',@(x,tc) ((r(randi(length(r), 1,length(x))).*(InitS*2))-InitS).*(StepDecayFactor^tc),...  %%rand uniform num in any direction add or subtracy (rand num -1 and +1 then add to x)   % x+((rand(1, length(x)).*2)-1)????   % (((rand(1, length(x)).*2.5)-1.25))
        'MaxStepSize', @(x,tc) ones(1, length(x)).*(max(abs(([0,1].*(InitS*2))-InitS))).*(StepDecayFactor^tc),...    %ones becuase rand's max value is 1
        'MinStepSize', @(x,tc) (ones(1, length(x)).*InitS.*(StepDecayFactor^tc))/2,...     
        'InitTemp',InitT,...  %1.7
        'MaxConsRej',20000,...  % 1000% of MaxTries, 2400, 1000
        'MaxSuccess',60,...   %  25% of MaxTries, 60,, 25
        'MaxTries',240,...    % controls constant nature of step size plot, 240, 100
        'StopTemp',1e-8,...
        'StopVal',-Inf,...
        'StopStepVal',Inf,...   %25000 Step at which the search is stopped
        'Verbosity',1);
...
elseif nargin<3, %user gave no options structure, use default
    ...
    fs = {'CoolSched','BasinFitness','Generator','StepSize', 'MaxStepSize','MinStepSize', 'InitTemp','MaxConsRej',...
...
min_stepSize= options.MinStepSize;
...
fval_all= ones(20000, 1)* (-1);% Relative frequency for all positions
...
while (InitS*StepDecayFactor^temp_counter)>0.1 
    ...
    %current_maxS=sqrt(sum(maxS.^2));  %Needed to to check if max stepsize is >1 (exploration is still possible)
    
    minS= min_stepSize(current, temp_counter);
    Min_S(counter)=  sqrt(sum(minS.^2)); 
    ...
    if (oldenergy-newenergy > 1e-6)
        ...
        if ((InitS*StepDecayFactor^temp_counter)>1)
           rf_acc(acc_counter) =  oldenergy- basinFitness(parent); %relative frequency of accepted (improving and disimproving) values only 
                                                                        % Excludes the first relative frequency but includes the last one (note last not not used in iteration!) 
                                                                        % Only when step size >1 (during exploration only) 
        end
